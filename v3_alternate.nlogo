...
trains-own [
  ...
  time-repaired
  to-service?
  forest-hill-yard-train?
  time-going-to-oak-grove
  time-checker
]
...
to go
  ...
  schedule-check ;; checks schedule to determine pattern of trains
  new-train-check ;; lets a train onto the line every 14 minutes
...
to setup-trains
  ...
  [
    ...
    set direction 0 ;; 0 is directionless, 1 is right, 2 is left
    set forest-hill-yard-train? false
  ]
  repeat 4 [
    ask train random 25 [
      setxy 20 0
      set forest-hill-yard-train? true
    ]
  ]
end
...
to station ;; station process
  ifelse (out-of-service? = true) [
    ifelse ([station-id] of patch-at 0 -1 = 3) or ([station-id] of patch-at 0 1 = 3) [
      set out-of-service? false
      set in-depot? true
    ]
    [
  ...
  [
    let check? false
    if ([station-id] of patch-at 0 1 = 20) [ ;; checks if Forest Hill is above the train, flips it to go other way
      if (miles-travelled > maintenance-cycle) [
        set out-of-service? true
      ]
      setxy xcor (ycor + 2)
      set direction 2
      set check? true
    ]
    if ([station-id] of patch-at 0 -1 = 1) [ ;; checks if Oak Grove is below the train
      if (miles-travelled > maintenance-cycle) [
        set out-of-service? true
      ]
      setxy xcor (ycor - 2)
...
to travel
  ifelse (travel-finish = false) [
    set time-travelling time-travelling + 1
    setxy xcor ycor
  ]
  [
    fd 1
    ifelse (next-station = 1) [
      set miles-travelled miles-travelled + (item 1 mileage-list)
    ]
    [
      set miles-travelled miles-travelled + (item (next-station - 2) mileage-list)
    ]
...
to-report travel-finish
  ifelse (to-service? = false) [
  ifelse (next-station = 1) [
     ifelse (time-travelling <= (item 0 schedule)) ;; Oak Grove exception
  [
    report false
  ]
  [
  ...
  [
    ifelse (time-travelling <= (item (next-station - 2) schedule)) ;; report if time travelling is not equal to the time between station
  [
    report false
  ]
  [
...
to schedule-check
  if (hours >= 6) and (hours < 9) and ((day-index <= 0) and (day-index >= 4))  [ ;; peak hour times
    set schedule time-list-weekday-peak
  ]
  if (day-index >= 0) and (day-index <= 4) [
    set schedule time-list-weekday
  ]
  if (day-index = 5) [
    set schedule time-list-saturday
  ]
  if (day-index = 6) [
    set schedule time-list-sunday
  ]
end

to depot
  setxy -14 3
  ifelse (maintenance-finish = false) [
    setxy xcor ycor
    set time-repaired time-repaired + 1
  ]
  [
    set in-depot? false
    set in-yard? true
...
to-report maintenance-finish
  ifelse (time-repaired != time-for-maintenance * 60 * 24) ;; report if time at station is not equal to waiting time
  [
    report false
  ]
  [
...
to new-train-check
  ...
  if (service-running? = true) [
    ifelse (minute-finish = false) [
      set time-checker time-checker + 1
    ]
    [
      ask train 24
        [
...
to go-to-oak-grove
  ifelse (going-finish = false) [
    set time-going-to-oak-grove time-going-to-oak-grove + 1
    setxy -17 2
  ]
  [
    set time-going-to-oak-grove 0
    set miles-travelled miles-travelled + (item 0 mileage-list + item 1 mileage-list)
    set to-service? false
    set in-service? false
    set out-of-service? false
    setxy -19 -1
    set at-station? true
...
to-report going-finish
  ifelse (time-going-to-oak-grove >= (item 0 schedule + item 1 schedule)) ;; report if going to oak grove has been achieved yet
  [
    report false
  ]
  [
...
to-report minute-finish
  ifelse (time-checker != 960) ;; report if going to oak grove has been achieved yet
  [
    report false
  ]
  [
