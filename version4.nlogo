...
to setup-patches
  ...
  ask patches [
    ...
    if ((pxcor = -9) or (pxcor = 9)) and (pycor = 0) [ ;; orange line connectors
      set pcolor orange
    ]
    if (pxcor = 9) and ((pycor = 2) or (pycor = 3)) [ ;; line to depot
      set pcolor orange
    ]
    if (pxcor = -4) or (pxcor = 4) and ((pycor = 1) or (pycor = -1)) [ ;; stations
      set pcolor green
    ]
    if (pxcor = -8) or (pxcor = 8) and ((pycor = 1) or (pycor = -1)) [ ;; stations
      set pcolor green
    ]
    if (pxcor = 0) and ((pycor = 1) or (pycor = -1)) [ ;; stations
      set pcolor green
    ]
    let x -8
    let id 0
    repeat 5 [
      ask patches with [pxcor = x and pycor = 0] [
        set pcolor item id color-list
        set plabel item id station-list
        set station-id id
        set station? true
      ]
      set x x + 4
    ...
    if (pxcor = 9) and (pycor = 4)[ ;; color depot blue
      set pcolor blue
      set plabel "depot"
    ]
...
to summon-trains
  create-trains 1 [
    ...
    setxy 9 4
    set in-depot? false ;; the train is in the depot when it starts
    set exiting-depot? true ;; the train is in the depot when it starts
    set go-to-depot? false ;; train is not in the process of going to the depot
    set entering-depot? false ;; the train is exiting
  ]
end

to summon-persons ;; creates the people in this simulation and assigns them a number to display
  if (open? = true)
  [
    ifelse ((hours >= 6) and (hours <= 9)) or ((hours >= 17) and (hours <= 20)) [
      every 0.25 [
    create-persons 1 [
      set shape "person"
      set label (word "passenger" " " who)
      set randall random 5 random-destination
    ...
    if (randall = 0) [
      set what-station 1 ;; this passenger will go to Baywater
      setxy -8 -2
    ]
    if (randall = 1) [
      set what-station 2 ;; this passenger will go to Stiltion St.
    ...
    if (randall = 2) [
      set what-station 3 ;; this passenger will go to Hodgepodge
    ...
    if (randall = 3) [
      set what-station 4 ;; this passenger will go to Superdome
        ifelse (destination = 4) [
          setxy 4 -2
        ]
        [
          setxy 4 2
    ...
    if (randall = 4) [
      set what-station 5 ;; this passenger will go to Thomas Park
      setxy 8 2
    ]
  ]
    ]
...
to random-destination ;; makes sure the destinations of all the passengers are not the same as their origin
  ask persons [
    set destination random 5
    while [destination-same] [set destination random 5]
  ]
end
...
to-report station-finish
  ifelse (time-at-station != 30) ;; report if time at station is not equal to waiting time
  [
    report false
  ]
  [
...
to go
  tick
  open-check ;; checks if system is open
  tick-clock ;; clock code
  summon-persons ;; spawns passengers
  ...
  if (day = "Monday") and (hours = 4) and (minutes = 0) and (seconds = 0) [ ;; records average passenger travel time per week
    week ;; code that disposes of passenger turtles to make way for new ones
  ]
  if (ticks = 2630000) [
...
to maintenance-check
  ask trains [
    if (miles-traveled > maintenance-cycle) and ((xcor = 9) and (ycor = 0)) [ ;; checks if maintenance cycle is surpassed by miles traveled
...
to exit-depot
  if (open? = true) [
  facexy 9 1 ;; face towards train line
  fd 3
  if (xcor = 9) and (ycor = 1) [
...
to train-movement
  ...
  if (go-to-depot? = true) [
    if (xcor = 9) and (ycor = 1) [
      facexy 9 4 ;; face towards the depot
...
to facing ;; faces train in a direction
  ...
  if (left? = true) [
    facexy -9 ycor ;; face left
  ]
  if (xcor = -9) and (ycor = 1) [ ;; if at left most position
    facexy xcor -1
  ]
  if (xcor = -9) and (ycor = -1) [ ;; switch left to right
    set right? true
    set left? false
  ]
  if (right? = true) [
    facexy 9 ycor ;; face right
  ]
  if (xcor = 9) and (ycor = -1) [ ;; if at right most position
    facexy xcor 1
  ]
  if (xcor = 9) and (ycor = 0) [ ;; switch right to left
    set right? false
    set left? true
  ]
end
...
to station
  ...
  ifelse (station-finish = false) [
      ...
      [
    let number 0
    ifelse ([station-id] of patch-at 0 -1 = 0) [
      set number 1
    ]
    [
    if ([station?] of patch-at 0 -1 = true) [
      set number [station-id] of patch-at 0 -1
    ]
    if ([station?] of patch-at 0 1 = true) [
      set number [station-id] of patch-at 0 1
    ]
    ...
    ask trains [
      set miles-traveled miles-traveled + item (number - 1) mileage-list
    ]
...
to passenger-boarding
  ask trains [
    if (at-station? = true) [ ;; asks train if it is at the station and if it is accepting more passengers
      ask persons [
        if (any? [trains-at 0 -1] of self) or (any? [trains-at 0 1] of self) [ ;; detects if train is in their station
          let number 0
          ask [trains-at 0 -1] of self [
            set number who
          ]
          ask [trains-at 0 1] of self [
            set number who
          ]
          set which-train number
...
to passenger-alighting
  ask persons [
    ...
    if (destination = 0) and ([plabel] of patch-at 0 -1 = "Baywater") [
      setxy -6 6
      set passenger-onboard? false
      set stop-count2? true
      set at-destination? true
    ]
    if (destination = 1) and (([plabel] of patch-at 0 -1 = "Stiltion St.") or ([plabel] of patch-at 0 1 = "Stiltion St.")) [
      setxy -6 -6
      set passenger-onboard? false
      set stop-count2? true
      set at-destination? true
    ]
    if (destination = 2) and (([plabel] of patch-at 0 -1 = "Hodgepodge") or ([plabel] of patch-at 0 1 = "Hodgepodge")) [
      setxy 6 -6
      set passenger-onboard? false
      set stop-count2? true
      set at-destination? true
    ]
    if (destination = 3) and (([plabel] of patch-at 0 -1 = "Superdome") or ([plabel] of patch-at 0 1 = "Superdome")) [
      setxy 6 6
      set passenger-onboard? false
      set stop-count2? true
      set at-destination? true
    ]
    if (destination = 4) and ([plabel] of patch-at 0 1 = "Thomas Park") [
      setxy -6 6
      set passenger-onboard? false
      set stop-count2? true
      set at-destination? true
...
to week ;; thank you to this thread on Google Groups for help in this output code https://groups.google.com/g/netlogo-users/c/Bq4MF0xg89c
  ...
  set total-average-list lput average total-average-list
...
to csv
  ...
  let directory "/Users/thomaspark/Documents/NetLogo/Maintenance Affected Simulation/version 2.5/" ;; local path directory
    let average-of-averages 0
    set average-of-averages mean total-average-list ;; takes average of all passenger travel times and sets it to the variable average
      let headers csv:to-row list "Maintenance Cycle" "Average"
  let export csv:to-row list maintenance-cycle average-of-averages

  file-open (word directory "averages4.csv")
  file-print export
  file-close
  stop
end
...
false
1
Polygon -2674135 true true 15 206 15 150 15 135 30 120 270 120 285 135 285 150 285 206 270 210 30 210
...
Polygon -7500403 true false 15 135
