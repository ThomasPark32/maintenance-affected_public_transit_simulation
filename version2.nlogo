globals [time time-in-depot miles-traveled time-at-station passenger-waiting? what-station destination waiting-time time-at-destination destination-waiting-time ]
turtles-own [in-depot? exiting-depot? normal-operation? left? right? at-station? randall passenger-onboard? at-destination?]
...
to setup-variables
  set waiting-time 50
  set destination-waiting-time 50
end

to summon-train
  create-trains 1
  ask trains [ set shape "train passenger car" set color orange ] ;; color the trains orange
  ask trains [
    set label "train"
    setxy 10 4
    set in-depot? true ;; the train is in the depot
  ]
end

to summon-person
  create-persons 1 ;; create a person
  ask persons [set shape "person" set label "passenger"]
  ask persons [set randall random 4]
  ask persons [set destination random 4]
  ask persons [
    if (randall = 0) [
...
to-report destination-finish
  ifelse (time-at-destination != destination-waiting-time) ;; report if time at station is not equal to waiting time
  [
    report false
  ]
  [
...
to passenger-check
  if any? persons [ ;; reports if there are any passengers
    set passenger-waiting? true
  ]
end

to destination-check

end

to depot
   ifelse (maintenance-check = false) [
...
to check-station
  if (what-station = 1) and (plabel = "station 1") [ ;; if the station is 1 and the label is station 1, stop
    set at-station? true
    set normal-operation? false
  ]
  if (what-station = 2) and (plabel = "station 2") [ ;; if the station is 2 and the label is station 2, stop
    set at-station? true
    set normal-operation? false
  ]
  if (what-station = 3) and (plabel = "station 3") [ ;; if the station is 3 and the label is station 3, stop
    set at-station? true
    set normal-operation? false
  ]
  if (what-station = 4) and (plabel = "station 4") [ ;; if the station is 4 and the label is station 4, stop
    set at-station? true
    set normal-operation? false
  ]
end
...
to check-destination
    if (destination = 0) and (plabel = "station 1") and (passenger-onboard? = true) [ ;; if the destination is 1, the label is station 1, and the passenger is onboard, stop
    set at-destination? true
    set normal-operation? false
  ]
  if (destination = 1) and (plabel = "station 2") and (passenger-onboard? = true) [ ;; if the destination is 2, the label is station 2, and the passenger is onboard, stop
    set at-destination? true
    set normal-operation? false
  ]
  if (destination = 2) and (plabel = "station 3") and (passenger-onboard? = true) [ ;; if the destination is 3, the label is station 3, and the passenger is onboard, stop
    set at-destination? true
    set normal-operation? false
  ]
  if (destination = 3) and (plabel = "station 4") and (passenger-onboard? = true) [ ;; if the destination is 4, the label is station 4, and the passenger is onboard, stop
    set at-destination? true
    set normal-operation? false
  ]
end
...
to destination-process
  ifelse (destination-finish = false) [
        set time-at-destination time-at-destination + 1 ;; increment timer by 1
        setxy xcor ycor  ;; stay at station
      ]
      [
        passenger-alight
        set at-destination? false ;; set destination flag to false
        set normal-operation? true ;; set normal operation flag true
        set time-at-destination 0
