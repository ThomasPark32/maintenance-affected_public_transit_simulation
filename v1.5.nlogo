globals [miles-travelled moving? maintenance-time at-station1? at-station2? picked-up? done? waiting-time ontrain-time iexist? total-time stopping-time]
...
to setup-globals
  set maintenance-time 0
end

to setup-train
  create-trains 1
  ask trains [ set shape "train passenger car" set color orange ]
  ask trains [
    setxy -8 0
...
to new-passenger
  create-persons 1
  ask persons [ set shape "person" set color black ]
  ask persons [
    set iexist? true
    setxy -8 1
    set label "person"
...
to train-movement
  if (miles-travelled >= maintenance-rate)[
    train-depot
  ]
  ask trains [
    fd 1
    ask patch-here [
      (ifelse
        pcolor = green [
          if (stopping-time <= 5)[
           ask trains [setxy -8 0]
            set stopping-time stopping-time + 1
          ]
        ]
        pcolor = yellow [
          waiting
...
to train-depot
  set moving? false
  set picked-up? false
  set maintenance-time maintenance-time + 1
  ask trains [setxy 0 -2]
  if (maintenance-time >= 50)[
    ask trains [setxy 0 0]
   set miles-travelled 0
    set maintenance-time 0
  ]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot miles-travelled"
